            ...
            <div className="flex flex-wrap gap-2">
                {sources.map((source, index) => (
                    <a
                        ...
                        className="text-xs bg-slate-600 text-sky-300 hover:bg-sky-500 hover:text-white rounded-full px-2.5 py-1 transition-colors duration-200 truncate"
                        title={source.title}
                    >
    ...
    return (
        <div className="space-y-6">
            {conversationHistory.length === 0 && !isRecording && (
                 <div className="text-center pt-12 pb-4 text-slate-400 animate-fade-in-slide-up">
                    <div className="inline-block p-4 bg-sky-500 rounded-full mb-4 shadow-lg shadow-sky-500/20">
                         <SautiIcon className="w-8 h-8" />
                    </div>
                    <h2 className="text-3xl font-bold text-slate-100 transition-all duration-300">{greeting}, {userName}!</h2>
                    <p className="text-slate-400 mt-2">Ready to chat in <span className="font-semibold">{currentMode} Mode</span>.</p>
                    <p className="text-sm text-slate-500 mt-4">Tap the microphone below to begin.</p>
                </div>
            )}
            {conversationHistory.map((message) => (
                <div key={message.id} className={`flex items-start gap-3.5 ${message.role === 'user' ? 'justify-end' : 'justify-start'} animate-fade-in-slide-up`}>
                   {message.role === 'model' && (
                    ...
                    <div className={`max-w-xl px-4 py-2.5 rounded-2xl ${message.role === 'user' ? 'bg-sky-500 text-white rounded-br-lg' : 'bg-slate-700 text-slate-200 rounded-bl-lg'}`}>
                        <p className="text-base whitespace-pre-wrap">{message.text}</p>
                        <SourceCitations sources={message.sources} />
                    </div>
                     {message.role === 'user' && (
                       <div className="w-9 h-9 rounded-full bg-slate-300 text-slate-600 flex items-center justify-center flex-shrink-0">
                           <UserIcon />
                       </div>
                   )}
                </div>
                ...
                <>
                    {_liveUserInput && (
                         <div className="flex items-start gap-3.5 justify-end animate-fade-in-slide-up">
                            <div className="max-w-xl px-4 py-2.5 rounded-2xl rounded-br-lg bg-sky-500/20 text-slate-300">
                    ...
                    {_liveModelOutput && (
                          <div className="flex items-start gap-3.5 justify-start animate-fade-in-slide-up">
                           <div className="w-9 h-9 rounded-full bg-slate-600 flex items-center justify-center flex-shrink-0">
                            ...
                            <div className="max-w-xl px-4 py-2.5 rounded-2xl rounded-bl-lg bg-slate-800 text-slate-400">
